...
export function createHealthRouter(config: AppConfig, healthService: HealthService) {
  ...
  router.get("/", (_req, res) => {
    res.status(200).json({ status: "ok", service: "winter-api", environment: config.NODE_ENV });
  });
  ...
  const readiness = async (_req: unknown, res: { status(code: number): typeof res; json(body: unknown): void }) => {
    try {
      ...
      res.status(200).json({ status: "ok", database: "connected" , message: "Hello World! This is the Winter API. It is running and ready to accept requests."});
    } catch {
