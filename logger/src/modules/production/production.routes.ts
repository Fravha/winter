...
import { ProductionController } from "./production.controller.js";
   import { catalogInputSchema, administrativeCatalogInputSchema, catalogUpdateSchema, idParamsSchema, sourceContainerParamsSchema, listSchema, customDefinitionSchema, customDefinitionUpdateSchema, customValueSchema, orderListSchema, productionOrderCreateSchema, transformationOrderCreateSchema, batchListSchema, containerCreateSchema, containerUpdateSchema, containerAssignSchema, containerTransferSchema, containerPartialTransferSchema, workListSchema, workCreateSchema, workCorrectionSchema, workInputSchema, workInputReversalSchema, grapeReceptionSchema, grapeReceptionListSchema, measurementCreateSchema, measurementListSchema, transformationCreateSchema, transformationListSchema, releaseBatchSchema, reverseReleaseSchema, measurementCorrectionSchema, receptionCorrectionSchema } from "./production.schema.js";
...
export function createProductionRouter(verifier: TokenVerifier, users: UserRepository, service: ProductionService) {
  ...
  router.get("/batches/:id/releases", ...auth, requirePermission("production:read"), validateRequest({ params: idParamsSchema }), controller.listReleases);
  router.get("/inventory-release/warehouses", ...auth, requirePermission("production:inventory_release"), controller.listInventoryReleaseWarehouses);
  router.post("/batches/:batchId/releases/:releaseId/reverse", ...auth, requirePermission("production:inventory_release_reverse"), validateRequest({ params: z.object({ batchId: z.string().uuid(), releaseId: z.string().uuid() }), body: reverseReleaseSchema }), controller.reverseRelease);
  ...
  router.post("/containers/:id/assign", ...auth, requirePermission("production:container_assign"), validateRequest({ params: idParamsSchema, body: containerAssignSchema }), controller.assignContainer);
  router.post("/containers/:sourceId/transfers", ...auth, requirePermission("production:container_transfer"), validateRequest({ params: sourceContainerParamsSchema, body: containerTransferSchema }), controller.transferContainer);
  router.post("/containers/:sourceId/transfers/partial", ...auth, requirePermission("production:container_transfer"), validateRequest({ params: sourceContainerParamsSchema, body: containerPartialTransferSchema }), controller.partialTransferContainer);
