...
export interface ProductionBatchTraceDto {
  ...
  releases: Array<{ releaseId: string; productionBatchId: string; quantity: string; unit: string; warehouseId: string; inventoryLotId: string; inventoryMovementId: string; operationKey: string; actorUserId: string; occurredAt: string; status: "ACTIVE"|"REVERSED"; reversal: { operationKey: string; reason: string; inventoryMovementId: string; occurredAt: string } | null }>;
...
export class ProductionTraceService {
  ...
  async get(id: string): Promise<ProductionBatchTraceDto> {
    ...
    return {
      ...
      releases: releases.map(row => ({ releaseId: row.id, productionBatchId: row.productionBatchId, quantity: decimal(row.quantity), unit: row.unit, warehouseId: row.warehouseId, inventoryLotId: row.inventoryLotId, inventoryMovementId: row.inventoryMovementId, operationKey: row.operationKey, actorUserId: row.actorUserId, occurredAt: iso(row.occurredAt), status: row.reversal ? "REVERSED" : "ACTIVE", reversal: row.reversal ? { operationKey: row.reversal.operationKey, reason: row.reversal.reason, inventoryMovementId: row.reversal.inventoryMovementId, occurredAt: iso(row.reversal.occurredAt) } : null })),
      receptions: receptions.map(row => ({ id: row.id, productionOrderId: row.productionOrderId, producerId: row.producerId, receivedAt: iso(row.receivedAt), status: row.status, observations: row.observations, version: row.version, items: row.items.map(item => ({ id: item.id, productionBatchId: item.productionBatchId, grapeVarietyId: item.grapeVarietyId, articuloId: item.articuloId, quantity: decimal(item.quantity), unit: item.unit })), corrections: row.corrections.map(correction) })),
       works: works.map(work => ({ id: work.id, productionOrderId: work.productionOrderId, transformationOrderId: work.transformationOrderId, workTypeId: work.workTypeId, performedAt: iso(work.performedAt), observations: work.observations, version: work.version, workType: { id: work.workType.id, code: work.workType.code, name: work.workType.name }, batches: work.batches.map(row => row.productionBatchId).sort(), containers: work.containers.map(row => row.productionContainerId).sort(), participants: work.participants.map(row => ({ participantId: row.productionParticipantId, role: row.role })), inputs: work.inputs.map(row => ({ id: row.id, productionWorkId: row.productionWorkId, articuloId: row.articuloId, article: articlesById.get(row.articuloId) ? { id: row.articuloId, codigo: articlesById.get(row.articuloId)!.codigo, nombre: articlesById.get(row.articuloId)!.nombre } : null, warehouseId: row.warehouseId, warehouse: row.warehouseId && warehousesById.get(row.warehouseId) ? { id: row.warehouseId, codigo: warehousesById.get(row.warehouseId)!.codigo, nombre: warehousesById.get(row.warehouseId)!.nombre } : null, inventoryLotId: row.inventoryLotId, quantity: decimal(row.quantity), unit: row.unit, inventoryMovementId: row.inventoryMovementId, operationKey: row.operationKey, status: row.reversedAt ? "REVERSED" : "ACTIVE", reversedAt: row.reversedAt?.toISOString() ?? null, reversalInventoryMovementId: row.reversalInventoryMovementId, reversalOperationKey: row.reversalOperationKey, reversalReason: row.reversalReason })), corrections: work.corrections.map(correction) })),
