...
export interface InventoryApi {
 ...
 registerProductionOutput(input: MovementInput & { lot?: LotInput }, context: ExecutionContext): Promise<unknown>;
  ...
  reverseProductionOutput(input: { warehouseId: string; articuloId: string; unit: string; quantity: string; inventoryLotId: string; idempotencyKey: string; reason: string }, context: TrustedIntermoduleContext, transaction: SharedTransactionContext): Promise<{ inventoryMovementId: string; inventoryLotId: string; warehouseId: string }>;
  isProductionOutputReversible(input: { warehouseId: string; articuloId: string; unit: string; quantity: string; inventoryLotId: string }, context: TrustedIntermoduleContext): Promise<boolean>;
  areProductionOutputsReversible(input: ReadonlyArray<{ warehouseId: string; articuloId: string; unit: string; quantity: string; inventoryLotId: string }>, context: TrustedIntermoduleContext): Promise<ReadonlyArray<boolean>>;
