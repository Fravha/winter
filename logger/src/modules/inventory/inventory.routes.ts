...
export function createInventoryRouter(verifier: TokenVerifier, users: UserRepository, service: InventoryService) {
 ...
 router.post("/lots/:inventoryLotId/classification", ...auth, requirePermission("inventory:lot_classify"), validateRequest({ body: classificationSchema }), c.command((b, x) => service.transitionInventoryLotClassification({ inventoryLotId: String(b.inventoryLotId), classification: b.classification as "PRODUCTO_ENVASADO" | "PRODUCTO_TERMINADO" | "PRODUCTO_TERMINADO_EXPORTACION", idempotencyKey: String(b.idempotencyKey) }, x)));
 router.post("/inbound", ...auth, requirePermission("inventory:inbound"), validateRequest({ body: movementSchema }), c.command((b, x) => service.registerInbound(b as unknown as MovementInput, x)));
 router.post("/outbound", ...auth, requirePermission("inventory:outbound"), validateRequest({ body: movementSchema }), c.command((b, x) => service.registerOutbound(b as unknown as MovementInput, x)));
 router.post("/transfer", ...auth, requirePermission("inventory:transfer"), validateRequest({ body: transferSchema }), c.command((b, x) => service.registerTransfer(b as unknown as TransferInput, x)));
 router.post("/adjustment", ...auth, requirePermission("inventory:adjust"), validateRequest({ body: adjustmentSchema }), c.command((b, x) => service.registerAdjustment(b as unknown as AdjustmentInput, x)));
 return router;
}