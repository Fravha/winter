...
import { z } from 'zod';
import { useCorrectProductionMeasurement } from '../../api/production.hooks';
...
import { ParticipantSelect } from '../shared/ProductionSharedSelects';
import { ScrollArea } from '@/components/ui/scroll-area';
import { toast } from '@/hooks/use-toast';
import { createOperationKey } from '../../utils/production-payload';
import type { ProductionMeasurement, MeasurementCorrectionInput } from '../../types/production.types';

interface Props {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  measurement: ProductionMeasurement;
  field: 'value' | 'unit' | 'measuredAt' | 'participantId' | 'observations';
  participantsMap: Map<string, string>;
}

export function CorrectMeasurementDialog({ open, onOpenChange, measurement, field, participantsMap }: Props) {
  const [step, setStep] = useState<'form' | 'summary'>('form');
  const [pendingPayload, setPendingPayload] = useState<MeasurementCorrectionInput | null>(null);
  const [definitiveError, setDefinitiveError] = useState(false);

  const schema = z.object({
    newValue: field === 'measuredAt' ? z.string().min(1, 'Requerido') :
              field === 'participantId' ? z.string().optional().or(z.literal('')) :
              field === 'value' ? z.string().regex(/^-?(?:0|[1-9]\d{0,11})(?:\.\d{1,6})?$/, 'Número inválido') :
              field === 'unit' ? z.string().trim().min(1, 'Requerido').max(100) :
              z.string().max(2000),
  ...
  const form = useForm({
    ...
    defaultValues: {
      newValue: field === 'measuredAt' ? toLocalDateTimeInput(measurement.measuredAt) :
                field === 'participantId' ? (measurement.participantId || '') :
  ...
  const handlePreview = async () => {
    ...
    if (field === 'unit') finalNewValue = values.newValue.trim();

    if ((field === 'observations' || field === 'participantId') && (!finalNewValue || finalNewValue.trim() === '')) {
      finalNewValue = null;
    }

    // Keep existing operationKey if it was just a network error retry. Generate new if definitive error or new correction.
    const opKey = (pendingPayload && !definitiveError) ? pendingPayload.operationKey : createOperationKey();

    const payload: MeasurementCorrectionInput = {
      field: field as any,
  ...
  const correctMutation = useCorrectProductionMeasurement({
    onSuccess: () => {
      toast({
        title: "Corrección Guardada",
        description: "El cambio se registró en la medición.",
      });
      onOpenChange(false);
  ...
  const handleSubmit = () => {
    ...
    correctMutation.mutate({ id: measurement.id, input: pendingPayload });
  };

  const formatValue = (val: string | null) => {
    if (!val) return 'N/A';
    if (field === 'measuredAt') return new Date(val).toLocaleString('es-BO');
    if (field === 'participantId') return participantsMap.get(val) || 'No disponible';
    return val;
  };

  const originalValue = field === 'measuredAt' ? measurement.measuredAt :
                        field === 'participantId' ? measurement.participantId :
                        field === 'value' ? measurement.value :
                        field === 'unit' ? measurement.unit :
                        measurement.observations;

  const fieldLabel = field === 'measuredAt' ? 'Fecha de Medición' :
                     field === 'participantId' ? 'Responsable' :
                     field === 'value' ? 'Valor' : 
                     field === 'unit' ? 'Unidad' : 'Observaciones';
        ...
        <DialogHeader className="p-6 pb-4 border-b">
          ...
          <DialogDescription>
            {step === 'form' 
              ...
              : 'Revise el cambio antes de confirmar.'}
          </DialogDescription>
        </DialogHeader>
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      {field === 'measuredAt' ? (
                        ...
                        <ParticipantSelect value={formField.value} onValueChange={formField.onChange} disabled={correctMutation.isPending} />
                      ) : field === 'value' ? (
                        <Input placeholder="0.00" {...formField} disabled={correctMutation.isPending} />
                      ) : field === 'unit' ? (
                        <Input placeholder="Unidad" {...formField} disabled={correctMutation.isPending} />
                      ) : (
