...
import { z } from 'zod';
import { useCorrectProductionWork } from '../../api/production.hooks';
...
import { WorkTypeSelect, TransformationOrderSelect } from '../shared/ProductionSharedSelects';
import { ScrollArea } from '@/components/ui/scroll-area';
import { toast } from '@/hooks/use-toast';
import type { ProductionWork, WorkCorrectionInput } from '../../types/production.types';

interface Props {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  work: ProductionWork;
  field: 'performedAt' | 'workTypeId' | 'transformationOrderId' | 'observations';
  typesMap: Map<string, string>;
  transformMap: Map<string, string>;
}

export function CorrectWorkDialog({ open, onOpenChange, work, field, typesMap, transformMap }: Props) {
  const [step, setStep] = useState<'form' | 'summary'>('form');
  const [pendingPayload, setPendingPayload] = useState<WorkCorrectionInput | null>(null);
  const [definitiveError, setDefinitiveError] = useState(false);

  const schema = z.object({
    newValue: field === 'performedAt' ? z.string().min(1, 'Requerido') :
              field === 'workTypeId' ? z.string().uuid('Seleccione tipo') :
              field === 'transformationOrderId' ? z.string().optional().or(z.literal('')) :
              z.string().max(2000),
  ...
  const form = useForm({
    ...
    defaultValues: {
      newValue: field === 'performedAt' ? toLocalDateTimeInput(work.performedAt) :
                field === 'workTypeId' ? work.workTypeId :
                field === 'transformationOrderId' ? (work.transformationOrderId || '') :
                (work.observations || ''),
      reason: ''
    }
  });
  ...
  const handlePreview = async () => {
    ...
    if ((field === 'observations' || field === 'transformationOrderId') && (!finalNewValue || finalNewValue.trim() === '')) {
      finalNewValue = null;
    }

    const payload: WorkCorrectionInput = {
      field: field as any,
      newValue: finalNewValue,
      reason: values.reason,
      // No operationKey needed for Works corrections per strict requirement
    };
  ...
  const correctMutation = useCorrectProductionWork({
    onSuccess: () => {
      toast({
        title: "Corrección Guardada",
        description: "El cambio se registró en el historial del trabajo.",
      });
      onOpenChange(false);
  ...
  const handleSubmit = () => {
    ...
    correctMutation.mutate({ id: work.id, input: pendingPayload });
  };
  ...
  const originalValue = field === 'performedAt' ? work.performedAt :
                        field === 'workTypeId' ? work.workTypeId :
                        field === 'transformationOrderId' ? work.transformationOrderId :
                        work.observations;

  const fieldLabel = field === 'performedAt' ? 'Fecha Ejecución' :
                     field === 'workTypeId' ? 'Tipo de Trabajo' :
                     field === 'transformationOrderId' ? 'Orden T.' : 'Observaciones';
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      {field === 'performedAt' ? (
                        ...
                        <WorkTypeSelect value={formField.value} onValueChange={formField.onChange} disabled={correctMutation.isPending} />
                      ) : field === 'transformationOrderId' ? (
                        <TransformationOrderSelect value={formField.value} onValueChange={formField.onChange} disabled={correctMutation.isPending} productionOrderId={work.productionOrderId} />
                      ) : (
