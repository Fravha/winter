...
import { TransformationOrderCreateDialog } from './TransformationOrderCreateDialog';
import { TransformationOrderDetailDialog } from './TransformationOrderDetailDialog';
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle } from '@/components/ui/alert-dialog';
import { TransformationOrderFilters, TransformationOrder } from '../../types/production.types';
import { useTransformationOrders, useCloseTransformationOrder } from '../../api/production.hooks';

export function TransformationOrderList() {
  const { can } = useAuth();
  const { toast } = useToast();
  const canCreate = can('production:transformation_order_create');
  const canClose = can('production:transformation_order_close');

  const [filters, setFilters] = useState<TransformationOrderFilters>({ page: 1, pageSize: 10 });
  const { data, isLoading, isFetching, error } = useTransformationOrders(filters);
  const closeMutation = useCloseTransformationOrder();

  const [createOpen, setCreateOpen] = useState(false);
  const [detailItem, setDetailItem] = useState<string | null>(null);
  const [confirmCloseItem, setConfirmCloseItem] = useState<TransformationOrder | null>(null);

  const handleClose = async () => {
    if (!confirmCloseItem) return;
    try {
      await closeMutation.mutateAsync(confirmCloseItem.id);
      toast({ title: 'Orden de transformación cerrada' });
    } catch (err) {
  ...
  // TransformationOrder doesn't directly have the full `productionOrder.code` in the list DTO by default unless the backend populates it.
  // The prompt says "Utilizar el productionOrder anidado cuando el API lo devuelva."
  // If it's not present, we just show the ID or a placeholder, but ideally backend sends it. Let's assume `(item as any).productionOrder?.code`
  const getPoCode = (item: any) => {
    if (item.productionOrder?.code) return item.productionOrder.code;
    return 'N/A'; // Since we shouldn't show UUID
  };

  const items: TransformationOrder[] = data?.data || [];
  ...
  return (
    <Card className="min-w-0">
      ...
      <CardContent>
        <div className="flex flex-wrap gap-4 items-center mb-4 justify-between">
          <div className="w-full sm:w-48">
            ...
            >
              <SelectTrigger aria-label="Filtro de estado" data-testid="select-filter-to-status">
            ...
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                      {filters.status ? 'No se encontraron órdenes de transformación para los filtros aplicados.' : 'No se encontraron órdenes de transformación.'}
                    </TableCell>
                  </TableRow>
                ) : (
                  items.map((item) => (
                    <TableRow key={item.id} data-testid={`row-transformation-order-${item.id}`}>
                      <TableCell className="font-mono font-medium">{item.code}</TableCell>
                      <TableCell className="font-mono">{getPoCode(item)}</TableCell>
                      <TableCell>
                      ...
                      <TableCell>{item.periodEnd ? formatDate(item.periodEnd) : '-'}</TableCell>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          <Button
                            ...
                            data-testid={`button-view-transformation-order-${item.id}`}
                          >
                          ...
                          {canClose && item.status === 'OPEN' && (
                            <Button
                              ...
                              data-testid={`button-close-transformation-order-${item.id}`}
                            >
      ...
      {createOpen && (
        <TransformationOrderCreateDialog
          open={createOpen}
      ...
      {detailItem && (
        <TransformationOrderDetailDialog
          orderId={detailItem}
          open={!!detailItem}
